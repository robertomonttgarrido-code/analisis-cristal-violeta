...
pal_deptos <- colorNumeric(
  ...
  domain = Departamentos_Honduras$`NÃ‚Â° casos GBG`
)

ui <- page_navbar(
  title = "Análisis Cristal Violeta",
  ...
  sidebar = sidebar(
    ...
    sliderInput("rango_fechas", 
                ...
                value = c(as.Date("2024-10-01"), as.Date("2026-03-31")), # Rango inicial seleccionado
                timeFormat = "%b %Y", # Muestra "oct 2024" en el slider
                step = 15,             # Salta de mes en mes (aprox 30 días)
                animate = TRUE),        # ¡Extra! Agrega un botón de "Play" para ver la evolución
...
server <- function(input, output, session) {
  ...
  datos_filtrados <- reactive({
    ...
    df <- df_trimestres %>%
      filter(
        `FECHA DE RECOLECCIÓN DE LA MUESTRA` >= input$rango_fechas[1],
        `FECHA DE RECOLECCIÓN DE LA MUESTRA` <= input$rango_fechas[2],
  ...
  output$mapa_cv <- renderLeaflet({
    leaflet(options = leafletOptions(zoomControl = FALSE)) %>%
      ...
      # Agregamos tus capas base de Honduras
      ...
      addPolygons(data = Departamentos_Honduras, 
                  ...
                  labelOptions = labelOptions(
                    style = list("font-size" = "13px")  # Cambia el tamaño del texto
                  ),
                  group = "Departamentos") %>%
      ...
      addMeasure(
        ...
        localization = "es" # Configura la herramienta en español
      )%>%
  ...
  observe({
    ...
    # Lógica de colores basada en tu columna 'Estatus CV'
  ...
  observe({
    ...
    leafletProxy("mapa_cv", data = df2) %>%
      ...
      addCircles(
        lng = ~LONGITUD,
        lat = ~LATITUD,
        radius = ~sqrt(`SUSCEPTIBLES`) * 100, # Usamos raíz cuadrada para que el área s, 
        color = "#B23AEE", # Color diferente para distinguir
        opacity = 0.9,
        fillOpacity = 0.4,
  ...
  output$grafico_barras <- renderPlotly({
    df_grafico <- datos_filtrados() %>%
      mutate(Mes = format(`FECHA DE RECOLECCIÓN DE LA MUESTRA`, "%Y-%m")) %>%
