...
poligonos_manuales <- readRDS(
  "data/poligonos_manuales.rds"
)
...
colores_honduras_ley<- c("Presencia CV"="red",
                         ...
                         "Fincas camaroneras no afiliadas"="orange",
                         "Fincas camaroneras ANDAH"="#CD1076")
...
colores_cuencas <- c(
  ...
  "Coco/Segovia" = "#C1CDC1",
  "Goascoran" = "#1f78b4",
  "Islas Cayos Pacifico" ="#EEEED1",
  "Nacaome" = "#33a02c",
  "Negro" = "#fd8d3c",
  "Sampile" = "#ffeda0"
)

pal_cuencas <- colorFactor(
  palette = colores_cuencas,
  domain = names(colores_cuencas),
  na.color = "gray"
)
...
pal_granjas <- colorFactor(
  palette = colores_granjas,
  domain = names(colores_granjas),
  na.color = "gray"
)
...
ui <- page_navbar(
  ...
  selectInput(
    ...
    label = "Tipo de finca muestreada:",
    choices = c(
      "Todos" = "Todos",
  ...
  output$mapa_cv <- renderLeaflet({
    leaflet(options = leafletOptions(zoomControl = FALSE)) %>%
      ...
      addPolygons(data = Departamentos_Honduras, 
                  ...
                  group = "Departamentos") %>%
      addPolylines(
        data = honduras_recortado,
        color = ~pal_cuencas(Cuenca),
        weight = 1.2,
        opacity = 0.5,
        group = "Hidrografia"
      ) %>%
      ...
      addCircles(data = fincas_camaroneras_general,
                 ...
                 label = ~paste("CUE:",`CUE`,
                                ...
                                "<br>Hectareas:", `Ha.`,
                                "<br>Afiliacion ANDAH:", `Afiliacion ANDAH`)%>%
                   lapply(htmltools::HTML),
                 labelOptions = labelOptions(
                   style = list("font-size" = "13px")
                 ),
                 color = ~pal_granjas(`Afiliacion ANDAH`),
                 fillColor =  ~pal_granjas(`Afiliacion ANDAH`),
                 opacity = 0.9,
                 fillOpacity = 0.6,
                 group = "Fincas camaroneras general")%>%
      addPolygons(
        data = poligonos_manuales,
        color = "blue",
        weight = 2,
        opacity = 0.9,
        fillColor = "#98F5FF",
        fillOpacity = 0.25,
        label = ~paste("Zona:",`nombre`)%>%
          lapply(htmltools::HTML),
        labelOptions = labelOptions(
          style = list("font-size" = "13px")
        ),
        group = "Zonas prioritarias"
      ) %>%
      ...
      addLegend(
        ...
        colors = unname(colores_cuencas),
        labels = names(colores_cuencas),
        title = '<div style="font-size: 13px; color: black;">Cuencas</div>',
        opacity = 1
      ) %>%
      addControl(
      ...
      addLayersControl(
        overlayGroups = c("Departamentos","Casos gusano barrenador",
                          ...
                          "Fincas camaroneras general","Zonas prioritarias"),  # Listado de capas controlables
    ...
    leafletProxy("mapa_cv", data = df) %>%
      ...
      addCircles(lng = ~`Longit final`, lat = ~`Latitud final`,
                 ...
                 label = ~paste0("<strong>Establecimiento:</strong> ", Establecimiento, 
                                 "<br><strong>Lugar muestreo:</strong> ", `Tipo de establecimiento`,
