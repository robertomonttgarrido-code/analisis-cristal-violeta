...
pal_deptos <- colorNumeric(
  ...
  domain = Departamentos_Honduras$`NÂ° casos GBG`
)

ui <- page_navbar(
  title = "AnÃ¡lisis Cristal Violeta",
...
server <- function(input, output, session) {
  ...
  datos_filtrados <- reactive({
    ...
    df <- df_trimestres %>%
      filter(
        `FECHA DE RECOLECCIÃN DE LA MUESTRA` >= input$rango_fechas[1],
        `FECHA DE RECOLECCIÃN DE LA MUESTRA` <= input$rango_fechas[2],
  ...
  output$mapa_cv <- renderLeaflet({
    leaflet(options = leafletOptions(zoomControl = FALSE)) %>%
      ...
      addPolygons(data = Departamentos_Honduras, 
                  color = "firebrick", 
                  weight = 1.5,
                  fillColor = ~pal_deptos(`NÂ° casos GBG`), 
                  fillOpacity = 0.2,
                  label = ~paste("Depto:",`DEPTO`,
                                 "<br>Num casos GBG:", `NÂ° casos GBG`)%>%
                    lapply(htmltools::HTML),
                  labelOptions = labelOptions(
                    style = list("font-size" = "13px")  # Cambia el tamaÃ±o del texto
                  ),
                  group = "Departamentos") %>%
      ...
      addMeasure(
        ...
        localization = "es" # Configura la herramienta en espaÃ±ol
      )%>%
  ...
  observe({
    ...
    # LÃ³gica de colores basada en tu columna 'Estatus CV'
  ...
  observe({
    ...
    leafletProxy("mapa_cv", data = df2) %>%
      ...
      addCircles(
        lng = ~LONGITUD,
        lat = ~LATITUD,
        radius = ~sqrt(`SUSCEPTIBLES`) * 100, # Usamos raÃ­z cuadrada para que el Ã¡rea s, 
  ...
  output$grafico_barras <- renderPlotly({
    df_grafico <- datos_filtrados() %>%
      mutate(Mes = format(`FECHA DE RECOLECCIÃN DE LA MUESTRA`, "%Y-%m")) %>%
